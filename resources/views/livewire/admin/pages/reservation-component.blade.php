...
<div>
		...
		<div class="container-fluid">
			<div class="form-head d-flex mb-3 align-items-start">
				<div class="mr-auto d-none d-lg-block">
					<h2 class="text-black font-w600 mb-0">Reservations</h2>
					<p class="mb-0">Here is your reservation list data</p>
				</div>
							...
							<tbody>
								@forelse ($reservations as $reservation)
									<tr>
										...
										<td>{{ $reservation->created_at->format('d F Y, H:i A') }}</td>
										<td>
											@if ($reservation->status == 'pending')
												<span class="btn btn-sm light btn-warning fs-16">{{ Str::title($reservation->status) }}</span>
											@elseif($reservation->status == 'completed')
												<span class="btn btn-sm light btn-success fs-16">{{ Str::title($reservation->status) }}</span>
											@elseif($reservation->status == 'cancelled')
												<span class="btn btn-sm light btn-danger fs-16">{{ Str::title($reservation->status) }}</span>
											@endif
										</td>
										...
										<td>
											<div class="dropdown ml-auto text-right">
												...
												<div class="dropdown-menu dropdown-menu-right">
													<a class="dropdown-item" data-toggle="modal" data-target="#editReservationModal" href="#!"
														wire:click='editReservation({{ $reservation->id }})'>
														<i class="las la-eye scale5 text-primary mr-2"></i>
														Edit Reservation
													</a>

													<a class="dropdown-item" data-toggle="modal" data-target="#deleteReservationModal" href="#!"
	...
	@include('livewire.admin.modals.reservations.create-reservation')
	@include('livewire.admin.modals.reservations.edit-reservation')

	<!-- Delete Modal -->
	<div wire:ignore.self id="deleteReservationModal" class="modal fade" tabindex="-1" role="dialog"
		aria-labelledby="deleteReservationModal" aria-hidden="true" style="display: none;">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title">Delete Reservations</h5>
					<button type="button" class="close" data-dismiss="modal"><span>&times;</span>
					</button>
				</div>
				<div class="modal-body">Are you sure you Want to delete reservations?</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-danger light" wire:click='resetForm' data-dismiss="modal">Close</button>
					<button type="button" class="btn btn-primary" data-dismiss="modal" wire:click='destroyReservation'>Yes</button>
				</div>
			</div>
		</div>
	</div>
